...
struct BST {
	...
	//Function (c)
	bool display(int, int);//traverse tree recursively and print content of every node
	void printTree(BTNode*, int& source, ostream&);

	//Function (d)
	bool CloneSubtree(BST, type);
	BTNode* finditem(BTNode*, type item);//use to find the student id
	void CloneSubChild(BTNode* founditem);//use to clone subtree into t2

	//Function (e)
	bool printLevelNodes(); //use for print out the nodes level by level
